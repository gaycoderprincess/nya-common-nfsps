...
public:
	static inline uint32_t IHandle = 0x402ED0;
	...
	virtual const UMath::Vector4 *GetWheelRoadNormal(unsigned int);
	...
	virtual float GetWheelEdgeOfControlSlipAngleLow(int);
	virtual float GetWheelEdgeOfControlSlipAngleHigh(int);
	virtual float GetWheelToleratedSlipRatio(int);
	virtual float GetWheelPeakLongForceSlipRatio(int);
	...
	virtual float GetHydraulicsCompression(unsigned int);
	virtual void ChangeSlot(bool);
	virtual bool IsSlotCar();
	virtual float GetWheelieAngle();
	virtual bool IsStaticResetCondition();
	virtual void SetWindCondition(float, float, float);
